...
-- Montar cabecalhos.
-- Explicar uma instruções complexa
-- Marcar onde realizamos uma alterações.
-- Marcar inicio e final de execução de uma regra de negócio.


Select * -- * significa todas as colunas 
from sys.objects 

sp_helptext sp_helpindex2 -- Execução da Procedure 
go
...
- Executar a instrução abaixo no banco de dados MASTER
...
@nOptions int = 1  , -- 0 (2)  is Default (Retorna dados do indice)
       ...
       */
	   ...
	   if @nOptions = 2   and @nOptionsResultTable & 8 = 8
 	   ...
 	   if @nOptions = 3 and @nOptionsResultTable & 8 = 8
 	      select * 
 	        from #tTMPFdBHelpIndex
 	        order by cTableName, iIndexID ,cAllocationType , nIndexLevel -- IDBUG [00010]
...
/*
Final da Procedure sp_helpindex2 
*/
